...
export const Route = createFileRoute("/course/$courseId")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.course.title ?? "Course"} · Course Designer` },
      {
        name: "description",
        content:
          "Backward-design cascade for the NST Entrepreneurship Track — COs, transfer tasks, assessment plan, rubric bands and forward handoff.",
      },
    ],
  }),
  notFoundComponent: () => (
    <main className="p-10">
      ...
      <Link to="/command-center" className="mt-3 inline-block font-mono text-xs text-primary">
  ...
  errorComponent: ({ error, reset }) => (
    <main className="p-10">
      ...
      <button onClick={reset} className="mt-3 font-mono text-xs text-primary">
        Retry
      </button>
    </main>
  ),
      ...
      <main className="relative flex-1 px-6 py-8 lg:px-10 lg:py-10 space-y-6">
        ...
        <section className="glass-strong relative overflow-hidden rounded-2xl p-6 lg:p-8">
          ...
          <p className="font-mono text-[11px] uppercase tracking-[0.32em] text-primary/80">
            Course · Semester {course.semester} · {course.credits}
          </p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Badge variant="outline" className="font-mono text-[10px]">
              {course.cos.length} COs · {course.weeklyPlan.length} weeks
            </Badge>
          </div>
        </section>
        ...
        <Tabs defaultValue="cos" className="w-full">
          <TabsList className="bg-background/40">
            <TabsTrigger value="cos" className="font-mono text-xs">
              <BookOpenCheck className="mr-2 h-3.5 w-3.5" />
              Course Outcomes
            </TabsTrigger>
            <TabsTrigger value="mapping" className="font-mono text-xs">
              <BookOpenCheck className="mr-2 h-3.5 w-3.5" />
              CO↔TLO Mapping
            </TabsTrigger>
            <TabsTrigger value="transfer" className="font-mono text-xs">
            ...
            <TabsTrigger value="assess" className="font-mono text-xs">
              <Gauge className="mr-2 h-3.5 w-3.5" />
              Assessment & Rubric
            </TabsTrigger>
            <TabsTrigger value="grader" className="font-mono text-xs">
              <Calculator className="mr-2 h-3.5 w-3.5" />
              Rubric Grader
            </TabsTrigger>
            <TabsTrigger value="handoff" className="font-mono text-xs">
            ...
            <div className="glass-strong rounded-2xl p-5">
              <div className="grid gap-2">
                {course.weeklyPlan.map((w) => (
                  <details
                    key={w.wk}
                    className="group rounded-lg border border-border/40 bg-background/30 p-3 open:border-primary/40"
                  >
                    <summary className="flex cursor-pointer items-center gap-3 font-mono text-xs">
                      <span className="flex h-7 w-9 items-center justify-center rounded-md bg-primary/15 text-primary">
                        W{w.wk}
                      </span>
          ...
          <TabsContent value="assess" className="mt-4 space-y-4">
            ...
            <div className="glass-strong rounded-2xl p-5">
              ...
              <p className="text-xs text-muted-foreground">
                Six performance bands per assessment — anti-gaming, behaviorally observable
                language.
              </p>
                    ...
                    <AccordionContent>
                      <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                        {r.bands.map((b, i) => (
                          <div
                            key={i}
                            className={`rounded-md border border-border/40 p-3 ${BAND_BG[i]}`}
                          >
          ...
          <TabsContent value="grader" className="mt-4">
            <RubricGrader
              assessments={course.assessments.map((a) => ({ name: a.name, weight: a.weight }))}
            />
          </TabsContent>
          ...
          <TabsContent value="handoff" className="mt-4">
            <div className="glass-strong rounded-2xl p-5">
              ...
              <p className="text-xs text-muted-foreground">
                What downstream courses receive when students complete this one.
              </p>
