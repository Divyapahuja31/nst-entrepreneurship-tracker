...
export const Route = createFileRoute("/founder-review")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Venture-grade founder evaluation rubric and scoring bands." },
    ],
  }),
  component: Page,
});
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="flex items-start gap-3">
              ...
              <div className="max-w-2xl">
                <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-primary/80">Portfolio-Style Review</p>
                <h2 className="font-mono text-xl tracking-tight">Standardised founder evaluation</h2>
                <p className="mt-2 text-sm text-muted-foreground">
                  Mirrors a venture-capital portfolio review. Every category is scored 1–10 against evidence,
                  and the aggregate places the founder in one of five bands.
                </p>
              </div>
            </div>
            <div className={`rounded-xl border px-5 py-4 ${band.tone}`}>
              <p className="font-mono text-[10px] uppercase tracking-[0.22em] opacity-80">Current Placement</p>
              <p className="mt-1 font-mono text-2xl">{avg.toFixed(1)}</p>
              <p className="mt-0.5 font-mono text-xs">{band.band} · {band.range}</p>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-2 md:grid-cols-5">
            {RUBRIC_BANDS.map((b) => (
              <div key={b.band} className={`rounded-lg border p-3 ${b.tone}`}>
                <p className="font-mono text-[10px] uppercase tracking-[0.22em] opacity-80">{b.range}</p>
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {FOUNDER_RUBRIC.map((r, i) => (
            <article key={r.category} className="glass-strong rounded-xl p-5">
              ...
              <Slider value={[scores[i]]} min={0} max={10} step={1} className="mt-3" onValueChange={(v) => { const n = [...scores]; n[i] = v[0]; setScores(n); }} />
              <div className="mt-3 flex flex-wrap gap-1">
                {r.examples.map((e) => (<Badge key={e} variant="outline" className="border-chart-2/30 bg-chart-2/10 font-mono text-[9px] text-chart-2">{e}</Badge>))}
              </div>
            </article>
          ))}
        </section>
